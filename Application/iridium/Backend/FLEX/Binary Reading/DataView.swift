...
struct DataView {
    ...
    func with(address: UInt, slid: Bool) throws -> DataView {
        let (target, start, end) = try info(forAddress: address, slid: slid)
    ...
    func withCString(offset: Int?, address: UInt, slid: Bool) throws -> DataView {
        ...
        let target = try info(forAddress: address, slid: slid).target
    ...
    func with<T>(offset: Int?, address: UInt, slid: Bool, targetType _: T.Type) throws -> DataView {
    ...
    private func info(forAddress address: UInt, slid: Bool) throws -> (target: UInt, start: UInt, end: UInt) {
        guard let slide, let start = startAddress, let end = endAddress else {
